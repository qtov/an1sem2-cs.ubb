...
*/
DrugController* createController(DrugRepo *repo, Node *undoList) {
...
int addDrug(DrugController* ctr, int id, char* name, char* concentration, int quantity) {
	...
	addListToUndo(&ctr->undo, ctr->repo->drugList);
...
void undoC(DrugController* ctr) {
	Node* p;
	p = doUndo(&ctr->undo);
	if (p != NULL) {
		destroyDrugList(ctr->repo->drugList);
		ctr->repo->drugList = p->data;
		free(p);
	}
}